<?php //var_dump($fields);  ?>
		<footer class="site-footer">
			<div class="sponsors">
				<div class="container">
					<div class="row">
					<?php //dynamic_sidebar('footer_widget'); ?>
						<ul>
							<li><a href="#" targert="blank"><img src="assets/images/trewa.png" alt=""></a></li>
							<li><a href="#" targert="blank"><img src="assets/images/trewa.png" alt=""></a></li>
							<li><a href="#" targert="blank"><img src="assets/images/trewa.png" alt=""></a></li>
							<li><a href="#" targert="blank"><img src="assets/images/trewa.png" alt=""></a></li>
						</ul>
					</div>
				</div>
			</div>
			
			<!-- </div> -->
			<div class="container">
				<div class="row">

					<ul class="social">
						
						<li class="icon-circle">
							<a href="#" target="blank">
				    	 		<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 612 612" style="enable-background:new 0 0 612 612;" xml:space="preserve">
									<g>
										<g>
											<path fill="ffffff" d="M612,306C612,137.004,474.995,0,306,0C137.004,0,0,137.004,0,306c0,168.995,137.004,306,306,306
												C474.995,612,612,474.995,612,306z M27.818,306C27.818,152.36,152.36,27.818,306,27.818S584.182,152.36,584.182,306
												S459.64,584.182,306,584.182S27.818,459.64,27.818,306z"/>
											<path fill="ffffff" d="M317.739,482.617V306h58.279l9.208-58.529h-67.487v-29.348c0-15.272,5.007-29.849,26.928-29.849h43.813v-58.418h-62.201
												c-52.298,0-66.569,34.438-66.569,82.175v35.413h-35.885V306h35.885v176.617H317.739L317.739,482.617z"/>
										</g>
									</g>

									</svg>
							</a>
			    	 	</li>

			    		<li class="icon-circle">
			    			<a href="#" target="blank">
			    	 		<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 290 290" style="enable-background:new 0 0 290 290;" xml:space="preserve">
							<g>
								<path fill="ffffff" d="M250.626,0H39.374C17.663,0,0,17.663,0,39.374v211.252C0,272.338,17.663,290,39.374,290h211.252
									C272.337,290,290,272.338,290,250.626V39.374C290,17.663,272.337,0,250.626,0z M145,215.846c-35.762,0-64.856-29.094-64.856-64.856
									c0-35.761,29.095-64.856,64.856-64.856s64.856,29.095,64.856,64.856C209.856,186.752,180.762,215.846,145,215.846z
									 M265.447,103.115H206.85C192.058,85.407,169.823,74.122,145,74.122s-47.058,11.286-61.85,28.993H24.553V46.804
									c0-14.313,11.645-25.957,25.958-25.957h188.979c14.313,0,25.958,11.644,25.958,25.957V103.115z"/>
								<path fill="ffffff" d="M145,109.296c-22.989,0-41.694,18.704-41.694,41.693c0,22.99,18.704,41.693,41.694,41.693s41.694-18.703,41.694-41.693
									C186.694,128,167.989,109.296,145,109.296z"/>
								<path fill="ffffff" d="M233.488,37.491h-25.093c-5.735,0-10.401,4.667-10.401,10.402v25.093c0,5.735,4.665,10.401,10.401,10.401h25.093
									c5.735,0,10.401-4.665,10.401-10.401V47.893C243.889,42.157,239.223,37.491,233.488,37.491z"/>
							</g>

							</svg>
							</a>
						</li>
			    	 	
			    	 	<li class="icon-circle">
			    	 		<a href="#" target="blank">
			    	 		<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
									viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
								<g>
									<g>
										<path fill="#ffffff" d="M256,0C114.609,0,0,114.609,0,256c0,141.391,114.609,256,256,256c141.391,0,256-114.609,256-256
											C512,114.609,397.391,0,256,0z M256,472c-119.297,0-216-96.703-216-216S136.703,40,256,40s216,96.703,216,216S375.297,472,256,472
											z"/>
										<path fill="#ffffff"  d="M320,208c-26.516,0-48,21.484-48,48c0,26.484,21.484,48,48,48c26.469,0,48-21.516,48-48C368,229.484,346.469,208,320,208z
											"/>
										<path fill="#ffffff" d="M192,208c-26.516,0-48,21.484-48,48c0,26.484,21.484,48,48,48c26.484,0,48-21.516,48-48C240,229.484,218.484,208,192,208z
											"/>
									</g>
								</g>

							</svg>
							</a>
			    	 	</li>

			    	 	<li class="icon-circle">
			    	 			<a href="#" target="blank">
			    	 		<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
									 viewBox="0 0 478.161 478.161" style="enable-background:new 0 0 478.161 478.161;" xml:space="preserve">
								<g>
									<path fill="#ffffff" id="Vimeo_2_" d="M408.173,31.91c-66.405-2.152-111.293,35.125-135.002,111.93c12.133-4.981,23.868-7.511,35.484-7.511
										c24.247,0,34.966,13.588,32.157,40.863c-1.395,16.576-12.114,40.584-32.157,72.123c-20.083,31.658-35.026,47.279-44.888,47.279
										c-12.97,0-24.725-24.307-35.464-73.119c-3.666-14.285-10.061-50.865-19.366-109.719c-8.607-54.571-31.479-80.013-68.597-76.367
										c-15.66,1.415-39.21,15.74-70.569,42.995C46.8,101.124,23.629,122.064,0,142.824l22.534,29.009
										C43.951,156.771,56.523,149.2,60.01,149.2c16.497,0,31.778,25.881,46.123,77.463c12.871,47.398,25.741,94.677,38.632,142.174
										c19.166,51.502,42.895,77.503,70.689,77.503c45.007,0,100.116-42.437,165.066-127.012c62.699-81.168,95.254-144.984,97.386-191.664
										C480.894,65.341,457.624,33.384,408.173,31.91z"/>
								</g>

							</svg>
							</a>
			    	 	</li>

			    	 	<li class="icon-circle">
			    	 		<a href="#" target="blank">
			    	 		<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"  viewBox="0 0 97.752 97.752" style="enable-background:new 0 0 97.752 97.752;"
									 xml:space="preserve">
								<g>
									<g>
										<path fill="ffffff" d="M27.251,58.126c4.926-0.242,9.883-0.268,14.704-1.49c0.017-0.072,0.032-0.146,0.049-0.22
											c-0.961-0.79-1.911-1.592-2.885-2.366c-3.271-2.6-6.161-5.55-8.37-9.124c-1.947-3.149-3.24-6.503-3.197-10.27
											c0.051-4.342,1.742-8.002,4.707-11.106c2.882-3.016,6.462-4.56,10.586-4.942c3.087-0.287,6.118-0.098,9.008,1.108
											c4.073,1.703,6.854,4.783,9.01,8.539c2.754,4.797,2.404,9.636,0.184,14.457c-2.803,6.087-7.423,10.496-13.131,13.855
											c-0.234,0.14-0.469,0.283-0.472,0.263c0.567,0.696,1.122,1.403,1.717,2.078c0.095,0.108,0.388,0.13,0.553,0.077
											c3.054-1.006,6.208-1.517,9.396-1.782c2.053-0.173,4.121-0.206,6.184-0.198c4.637,0.02,21.949,0.438,31.652,0.678
											c0.521-2.856,0.807-5.798,0.807-8.807C97.75,21.884,75.868,0.001,48.875,0.001S0,21.884,0,48.876c0,2.646,0.218,5.243,0.623,7.775
											C8.209,57.272,23.099,58.33,27.251,58.126z"/>
										<path fill="ffffff" d="M48.312,62.603c-3.041,1.953-5.233,4.58-6.018,8.166c-0.572,2.621,0.585,4.713,2.93,5.506
											c0.841,0.283,1.761,0.334,2.486,0.461c0.988-0.115,1.812-0.133,2.59-0.327c0.822-0.204,1.664-0.472,2.39-0.897
											c2.146-1.257,2.637-3.412,1.463-6.072c-1.172-2.664-3.105-4.758-5.13-6.773C48.889,62.53,48.476,62.497,48.312,62.603z"/>
										<path fill="ffffff" d="M69.588,59.49c-3.38-0.089-6.77,0.021-10.148,0.157c-2.617,0.104-5.213,0.463-7.711,1.322
											c-0.123,0.042-0.242,0.102-0.467,0.196c0.266,0.287,0.479,0.517,0.688,0.751c2.146,2.378,3.934,4.965,4.901,8.066
											c1.144,3.67-0.313,6.215-3.278,7.99c-5.339,3.196-12.341,0.147-13.685-5.932c-0.677-3.064,0.327-5.698,2.354-7.938
											c1.08-1.191,2.386-2.182,3.594-3.258c0.186-0.166,0.395-0.303,0.678-0.518c-0.748-0.648-1.428-1.268-2.148-1.836
											c-0.133-0.106-0.447-0.071-0.641,0c-4.674,1.718-9.556,2.201-14.476,2.345c-4.399,0.129-8.804,0.123-13.206,0.136
											c-0.859,0.002-9.551-0.851-14.846-1.361c4.896,21.826,24.376,38.139,47.678,38.139c23.034,0,42.334-15.939,47.503-37.386
											C86.959,60.04,73.232,59.582,69.588,59.49z"/>
										<path fill="ffffff" d="M60.062,38.107c0.72-2.602,0.541-5.817-0.867-8.186c-2.01-3.38-4.566-6.339-8.152-7.958
											c-2.168-0.979-11.062-3.373-17.355,3.55c-2.588,2.68-4.037,5.838-3.523,9.627c0.459,3.383,1.888,6.457,3.752,9.259
											c2.802,4.208,6.563,7.523,10.528,10.61c0.362,0.283,0.636,0.233,0.997,0.029c2.654-1.502,5.103-3.285,7.287-5.41
											C56.077,46.369,58.794,42.69,60.062,38.107z"/>
									</g>
								</g>

								</svg>
							</a>
			    	 	</li>
			    	</ul> 

			    </div>
			</div>

			<div class="arrow" >
				<a href="#scroll-hero" class="anchor">
			        <span class="top"></span>
			    </a>
		    </div>

			<!-- <div class="wm">
			      <a href="http://www.julianweblab.com" target="blank" class="anchor" data-toggle="tooltip" data-placement="right" title="DESIGNED BY <a href='http://wwwjulianweblab.com' target='blank'>www.julianweblab.com">
			       <img src="assets/images/julian.png">
			      </a>
		    </div> -->
		</footer>


	</body>
</html>
